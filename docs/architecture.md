...
- callback coordinates for events, audit and approval requests, plus a
  per-run callback credential

The callback credential is generated by Laravel, included only in the signed
envelope, and sent by the runtime on every callback request. Laravel stores
only its SHA-256 hash and accepts it only while the run is queued or running;
completed and failed runs reject callbacks, so the credential cannot outlive
the run. It is never exposed to the browser.
